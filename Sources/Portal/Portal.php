...
function Portal($doinit = false)
{
	...
	if((!empty($rqaction) && in_array($rqaction, array('jsmodify', 'post2')) && !isset($_REQUEST['preview']))
	...
	if(!empty($pmxadmfunc) && in_array($_GET[$pmxadmfunc], array('on', 'off')) && !empty($modSettings['portal_enabled']))
	{
		...
		{
			$logCook = get_cookie($cookiename);
			$sessID =  get_cookie('PHPSESSID');
			$tmp = json_encode(array($logCook, $sessID), true);
			set_cookie($pmxadmfunc, $tmp);
		...
		elseif($pmxadmfunc == 'spidertest' && $_GET[$pmxadmfunc] == 'off')
		{
			$udata = get_cookie($pmxadmfunc);
			set_cookie($pmxadmfunc, '');
			if(isset($udata) && ($tmp = pmx_json_decode($udata, true)))
			{
				set_cookie('PHPSESSID', '');
				set_cookie($cookiename, '');
				setECL_Cookie();
				set_cookie('PHPSESSID', $tmp[1]);
				set_cookie($cookiename, $tmp[0]);
				unset($tmp);
				unset($udata);
			}
			unset($_GET[$pmxadmfunc]);
	...
	var pmx_restore_speed = '. intval($context['pmx']['settings']['restorespeed']) .';');
