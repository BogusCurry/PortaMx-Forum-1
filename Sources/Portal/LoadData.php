...
function PortaMx_loadCompressed($file, $path = array(), $isInline = false)
{
	...
	if(!empty($modSettings['isMobile']) && $ext == '.css')
		$minfile = str_replace($ext, '', $file) .'.mob.min'. $ext;
	else
		...
		if(($curTime = $pmxCacheFunc['get']('minimized_'. $file)) === null)
		{
			...
			else
			{
				...
				if(!empty($modSettings['isMobile']))
					$minimized = str_replace(':hover', ':active', $minimized);
			}
...
function PortaMx_getSettings()
{
	...
	$context['pmx_syscssdir'] = $settings['default_theme_dir'] .'/Portal/SysCss/';
	...
	$context['pmx_syscssurl'] = $settings['default_theme_url'] .'/Portal/SysCss/';
...
function PortaMx_getTitle($cfg)
{
	...
	if(isset($cfg['title'][$context['pmx']['currlang']]) && !empty($cfg['title'][$context['pmx']['currlang']]))
		$title = htmlspecialchars($cfg['title'][$context['pmx']['currlang']], ENT_QUOTES);
	elseif(isset($cfg['title'][$language]) && !empty($cfg['title'][$language]))
		$title = htmlspecialchars($cfg['title'][$language], ENT_QUOTES);
	elseif(isset($cfg['title']['english']) && !empty($cfg['title']['english']))
		$title = htmlspecialchars($cfg['title']['english'], ENT_QUOTES);
	return $title;
}
...
function PortaMx_makeSafeContent($content, $type = '')
{
	...
	if(in_array($type, array('html', 'bbc_script', 'script', 'code', 'bbc')))
	{
		$res = true;
		while($res)
		{
...
function PortaMx_SmileyToBBC($content)
{
	...
	if(!empty($content))
	{
		...
		if(preg_match_all('~<img.*'. preg_quote($modSettings['smileys_url'] .'/portamx/') .'[^>]*>~U', $content, $match) > 0)
		{
...
function PortaMx_headers($action = '')
{
	...
	{
		if(!in_array($name, array('PHPSESSID', 'eclauth', 'pmx_YOfs', 'pmx_YOfs2', $cookiename, 'DBGSESSID')))
		{
			$tmp[0] .= '\''. $name .'\',';
			$tmp[1] .= '\''. $value .'\',';
		}
	...
	var pmx_onForum = '. (!empty($_GET['board']) ? 'true' : 'false') .';
	...
	if(!empty($context['browser']['is_edge']))
		addInlineCss('
	.pwindicon{top:-23px !important;}
	.ddImage{top:-18px !important;}');
	elseif(!empty($context['browser']['is_ie11']) || !empty($context['browser']['is_firefox']))
		addInlineCss('
	...
	if (target.tagName == "IMG" || (target.tagName == "A" && target.firstChild.tagName == "IMG")) return false;}
	...
	aSwappableContainers: [
		...
		oCookieOptions: {
			bUseCookie: true,
			sCookieName: \'pmx_upshr'. $pname .'Panel\',
