...
function Portal_allow_guest($board, $topic)
{
	...
	if(!empty($modSettings['portal_enabled']) && (empty($board) && empty($topic)) || Portal_frontactions())
		return 'PortaMx';
	else
		return 'KickGuest';
}
					...
					'areas' => array(
						...
						'pmx_blocks' => array(
							...
							'subsections' => array(
								...
								'bib' => array($txt['pmx_admBlk_panels']['bib']),
							),
						),
						'pmx_articles' => array(
...
function Portal_MenuContext(&$menudata)
{
	...
	if(strpos($_SERVER['QUERY_STRING'], 'xml') === false && strpos($_SERVER['QUERY_STRING'], 'popup') === false)
	{
		...
		// not Admin, lock for moderators
		else
		{
			if(allowedTo('profile_view_own') && allowPmx('pmx_admin, pmx_blocks, pmx_articles, pmx_create', true))
			{
				$fnd = array_search('moderate', array_keys($menudata));
				if($fnd === false)
					$fnd = array_search('calendar', array_keys($menudata));
				if($fnd === false)
				...
				array(
					'portal' => array(
						'title' => $txt['pmx_extension'],
						'href' => $scripturl . '?action=portal;area=pmx_center;'. $context['session_var'] .'='. $context['session_id'],
						'icon' => '<span class="generic_icons portal"></span>',
						'active_button' => false,
						'show' => allowPmx('pmx_admin', true),
						'sub_buttons' => array(
							'pmxsettings' => array(
								'title' => $txt['pmx_settings'],
								'href' => $scripturl . '?action=portal;area=pmx_settings;'. $context['session_var'] .'='. $context['session_id'],
								'show' => allowPmx('pmx_admin', true),
							),
							'pmxblocks' => array(
								'title' => $txt['pmx_blocks'],
								'href' => $scripturl . '?action=portal;area=pmx_blocks;'. $context['session_var'] .'='. $context['session_id'],
								'show' => allowPmx('pmx_admin, pmx_blocks', true),
							),
							'pmxarticles' => array(
								'title' => $txt['pmx_articles'],
								'href' => $scripturl . '?action=portal;area=pmx_articles;'. $context['session_var'] .'='. $context['session_id'],
								'show' => allowPmx('pmx_admin, pmx_articles, pmx_create', true),
							),
							'pmxcategories' => array(
								'title' => $txt['pmx_categories'],
								'href' => $scripturl . '?action=portal;area=pmx_categories;'. $context['session_var'] .'='. $context['session_id'],
								'show' => allowPmx('pmx_admin', true),
							),
						),
					)
				...
				foreach($menudata['portal']['sub_buttons'] as $button => $value)
				{
					if(empty($value['show']))
						unset($menudata['portal']['sub_buttons'][$button]);
				}
			}
		}
		...
		**/
		...
		if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'portal' && allowPmx('pmx_admin, pmx_blocks, pmx_articles, pmx_create', true))
