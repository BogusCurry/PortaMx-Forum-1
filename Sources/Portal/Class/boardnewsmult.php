...
class pmxc_boardnewsmult extends PortaMxC_SystemBlock
{
	...
	*/
	function fetch_data()
	{
		global $pmxcFunc, $context, $scripturl, $settings, $modSettings, $txt;
		...
		if(!empty($boards))
		{
			...
			while ($row = $pmxcFunc['db_fetch_assoc']($request))
			{
				...
				if(!empty($max))
				{
					...
					$this->posts[$row['id_msg']] = array(
						...
						'board' => array(
							'link' => '<a href="'. $scripturl .'?board='. $row['id_board'] .'.0#top">'. $row['name'] .'</a>',  
						),
					);
				}
			}
			...
			if(!is_null($this->posts))
			{
				...
				foreach($this->posts as $mid => $post)
				{
					...
					$post['body'] = parse_bbc($post['body'], $post['smileys_enabled'], $post['id_msg']);
				...
				if(isset($context['lbimage_data']['lightbox_id']))
					echo '
										<a href="" data-link="'. $scripturl .'?action=dlattach;topic='. $post['id_topic'] .'.0;attach='. $data['id_attach'] .';image" title="'. $txt['pmx_hs_expand'] .'" data-lightbox="'. $context['lbimage_data']['lightbox_id'] .'" data-title="'. $data['filename'] .'" class="pmxhs_img">
											<img src="'. $scripturl .'?action=dlattach;topic='. $post['id_topic'] .'.0;attach='. (isset($data['id_thumb']) ? $data['id_thumb'] : $data['id_attach']) .';image" alt="'. $data['filename'] .'"'. $style .' />
										</a>';
				else
					echo '
										<img src="'. $scripturl .'?action=dlattach;topic='. $post['id_topic'] .'.0;attach='. (isset($data['id_thumb']) ? $data['id_thumb'] : $data['id_attach']) .';image" alt="'. $data['filename'] .'"'. $style .' class="pmxhs_img" oncontextmenu="return false" />';
