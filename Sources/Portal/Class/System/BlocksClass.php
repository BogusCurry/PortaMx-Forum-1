...
class PortaMxC_Blocks
{
	...
	*/
	function __construct($blockconfig, &$visible)
	{
		...
		// check active and if ecl set
		$active = !empty($this->cfg['active']) || (!empty($context['pmx']['inAdmin']) && isset($_REQUEST['area']) && $_REQUEST['area'] == 'pmx_blocks');
		$visible = !empty($active) && (empty($this->cfg['config']['check_ecl']) || !empty($this->cfg['config']['check_ecl']) && checkECL_Cookie(!empty($this->cfg['config']['check_eclbots'])));
		...
		if(!empty($visible))
		{
			...
			if($visible && empty($this->inBlockCall))
			{
				...
				if($visible && $context['pmx']['settings']['frontpage'] != 'none' && $this->cfg['side'] == 'front')
					$context['pmx']['showhome'] += (empty($context['pmx']['inAdmin']) ? intval(!empty($visible)) : intval(!empty($this->cfg['active'])));
		...
		{
			if(!empty($this->cfg['config']['settings']['split']))
			{
	...
	*/
	function getUserTitle($cfg = null, $fallback = '')
	{
		...
		if(is_null($cfg))
			$titles = isset($this->cfg['config']['title']) ? $this->cfg['config']['title'] : '';
		else
			$titles = isset($cfg['config']['title']) ? $cfg['config']['title'] : '';

		if(isset($titles[$context['pmx']['currlang']]) && !empty($titles[$context['pmx']['currlang']]))
			return htmlspecialchars($titles[$context['pmx']['currlang']], ENT_QUOTES);
		elseif(isset($titles[$language]) && !empty($titles[$language]))
			return htmlspecialchars($titles[$language], ENT_QUOTES);
		elseif(isset($titles['english']) && !empty($titles['english']))
