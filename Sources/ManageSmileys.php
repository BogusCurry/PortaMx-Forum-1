		...
		$listOptions = array(
			...
			'columns' => array(
				...
				'tooltip' => array(
					...
					'data' => array(
						'function' => function ($rowData) use ($modSettings, $context, $txt, $pmxcFunc)
						{
							if (empty($modSettings['smileys_dir']) || !is_dir($modSettings['smileys_dir']))
...
function InstallSmileySet()
{
	...
	if (isset($_REQUEST['set_gz']))
	{
		...
		// Check that the smiley is from portamx.com, for now... maybe add mirroring later.
		if (preg_match('~^http://[\w_\-]+\.portamx\.com/~', $_REQUEST['set_gz']) == 0 || strpos($_REQUEST['set_gz'], 'dlattach') !== false)
			fatal_lang_error('not_on_portamx');
