...
function getFileVersions(&$versionOptions)
{
	...
	$sourceFiles = array('' => $sourcedir, 'Portal/' => $sourcedir .'/Portal', 'Portal/Class/' => $sourcedir .'/Portal/Class', 'Portal/Class/System/' => $sourcedir .'/Portal/Class/System');
	foreach($sourceFiles as $prefix => $source)
	{
		$source_dir = dir($source);
		while ($entry = $source_dir->read())
		{
			if (substr($entry, -4) === '.php' && !is_dir($source .'/'. $entry) && $entry !== 'index.php')
			{
				// Read the first 4k from the file.... enough for the header.
				$fp = fopen($source . '/' . $entry, 'rb');
				$header = fread($fp, 4096);
				fclose($fp);

				// Look for the version comment in the file header.
				if (preg_match('~\*\s@version\s+(.+)[\s]{2}~i', $header, $match) == 1)
					$version_info['file_versions'][$prefix . $entry] = $match[1];
				// It wasn't found, but the file was... show a '??'.
				else
					$version_info['file_versions'][$prefix . $entry] = '??';
			}
		}
		$source_dir->close();
	}
	...
	$directories = array('default_template_versions' => array('' => $settings['default_theme_dir'], 'Portal/' => $settings['default_theme_dir'] .'/Portal'));

	if ($settings['theme_id'] != 1)
		$directories += array('template_versions' => $settings['theme_dir']);

	foreach ($directories as $type => $dirnames)
	{
		foreach ($dirnames as $prefix => $dirname)
		{
			...
			while ($entry = $this_dir->read())
			{
				if (substr($entry, -12) == 'template.php' && !is_dir($dirname . '/' . $entry))
				{
					...
					// Look for the version comment in the file header.
					if (preg_match('~\*\s@version\s+(.+)[\s]{2}~i', $header, $match) == 1)
						$version_info[$type][$prefix . $entry] = $match[1];
					// It wasn't found, but the file was... show a '??'.
					else
						$version_info[$type][$prefix . $entry] = '??';
				}
			}
			$this_dir->close();
		}
	...
	$lang_directories = array('' => $lang_dir, 'Portal/' => $lang_dir .'/Portal');
	foreach ($lang_directories as $prefix => $dirname)
	{
		$curdir = dir($dirname);
		while ($entry = $curdir->read())
		{
			if (substr($entry, -4) == '.php' && $entry != 'index.php' && !is_dir($dirname . '/' . $entry))
			{
				// Read the first 768 bytes from the file.... enough for the header.
				$fp = fopen($dirname . '/' . $entry, 'rb');
				$header = fread($fp, 768);
				...
				if (preg_match('~(?://|/\*)\s*Version:\s+(.+?);\s*' . preg_quote($name, '~') . '(?:[\s]{2}|\*/)~i', $header, $match) == 1)
					$version_info['default_language_versions'][$language][$prefix . $name] = $match[1];
				// It wasn't found, but the file was... show a '??'.
				else
					$version_info['default_language_versions'][$language][$prefix . $name] = '??';
			}
		}
		$curdir->close();
	}
...
function updateAdminPreferences()
{
	...
	$options['admin_preferences'] = json_encode($context['admin_preferences'], true);
