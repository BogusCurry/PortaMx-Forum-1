...
{
	global $pmxCacheFunc, $modSettings, $scripturl;
	...
	if(isset($_REQUEST['jscook']))
	{
		...
		switch ($_REQUEST['mode'])
		{
			...
			case 'syntax';
				$result = PortaMx_PHPsyntax(pack('H*', $_REQUEST['value']));
			break;
		}
		...
		if(!empty($_SERVER['REMOTE_ADDR']))
		{
			...
			if(is_array($CachedCooks))
			{
				foreach($CachedCooks as $key => $val)
					setcookie($key, $val, 0, $modSettings['pmx_cookparts']['path'], $modSettings['pmx_cookparts']['host'], !empty($modSettings['secureCookies']), !empty($modSettings['httponlyCookies']));
				$data = $pmxCacheFunc['put']($cName, null);
			}
		}
	}
}
...
function set_cookie($name, $value, $time = 0)
{
	...
	if(checkECL_Cookie())
	{
		...
		{
			setcookie($name, '', time() - 300, $modSettings['pmx_cookparts']['path'], $modSettings['pmx_cookparts']['host'], !empty($modSettings['secureCookies']), !empty($modSettings['httponlyCookies']));
			if(isset($_COOKIE[$name]))
				unset($_COOKIE[$name]);
		}
		else
			setcookie($name, $value, (!empty($time) ? intval($time) : 0), $modSettings['pmx_cookparts']['path'], $modSettings['pmx_cookparts']['host'], !empty($modSettings['secureCookies']), !empty($modSettings['httponlyCookies']));
	}

	// no ecl accept .. store coookies the the cache (name is the users IP)
	elseif(!empty($_SERVER['REMOTE_ADDR']))
	{
		...
		if(isset($data[$name]) && ($value == '' || is_null($value)))
			unset($data[$name]);
		elseif(is_array($data) && array_search($name, array_flip($data), true) !== false)
			$data[$name] = $value;
		else
			$data = (is_null($data) ? array($name => $value) : array_merge($data, array($name => $value)));
...
function ecl_loadvars($overlay)
{
	...
	var eclInMaintenace = '. (!empty($maintenance) || !empty($overlay) ? 'false' : 'false') .';
	var eclofsTop = '. intval($modSettings['ecl_topofs']) .';
	var eclOverlay = '. (!empty($maintenance) || !empty($overlay) ? 'false' : 'false') .';');
		...
		$codes = array(
			...
			array(
				...
				'validate' => function (&$tag, &$data, $disabled) use ($modSettings, $sourcedir, $txt)
				{
					...
					{
						$width = $height = $alt = $class = $noLightbox = $totop = '' ;
						...
						if (isset($_REQUEST['preview']) || !isset($context['lbimage_data']['lightbox_id']) || !empty($noLightbox))
						{
							if (!empty($modSettings['attachmentShowImages']))
								$returnContext = '<img style="vertical-align:top" src="'. ($currentAttachment['thumbnail']['has_thumb'] ? $currentAttachment['thumbnail']['href'] : $currentAttachment['href']) .'"'. $alt . $width . $height . $class .' oncontextmenu="return false">';
						}
						else
						{
							if (!empty($modSettings['attachmentShowImages']))
								$returnContext = '<a style="vertical-align:top" class="lb-link '. $class .'" href="" data-link="'. $currentAttachment['href'] .'" title="'. $txt['lightbox_expand'] .'" data-lightbox="'. $context['lbimage_data']['lightbox_id'] .'" data-title="'. $currentAttachment['name'] .'"><img src="'. ($currentAttachment['thumbnail']['has_thumb'] ? $currentAttachment['thumbnail']['href'] : $currentAttachment['href']) .'"'. $alt . $width . $height . $class .' oncontextmenu="return false"></a>';
						}
					}
			...
			array(
				...
				'validate' => function (&$tag, &$data, $disabled)
				{
					...
					if (isset($_REQUEST['preview']) || !isset($context['lbimage_data']['lightbox_id']) || !empty($noLightbox))
						$data[0] = '<img style="vertical-align:top" src="'. $data[0]  .'"'. $alt . $width . $height . $class .' oncontextmenu="return false">';
					else
						$data[0] = '<a style="vertical-align:top" class="lb-link" href="" data-link="'. $data[0] .'" title="'. $txt['lightbox_expand'] .'" data-lightbox="'. $context['lbimage_data']['lightbox_id'] .'" data-title="'. substr($data[0], strrpos($data[0], '/')+1) .'" oncontextmenu="return false"><img src="'. $data[0]  .'"'. $alt . $width . $height . $class .'></a>';
				},
				'disabled_content' => '$1',
			),
			array(
				...
				'validate' => function (&$tag, &$data, $disabled)
				{
					...
					if (isset($_REQUEST['preview']) || !isset($context['lbimage_data']['lightbox_id']))
						$data = '<img style="vertical-align:top" src="'. $data  .'"'. $class .' oncontextmenu="return false">';
					else
