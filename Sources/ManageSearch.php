...
function CreateMessageIndex()
{
		...
		else
		{
			...
			while (time() < $stop)
			{
				...
				if ($num_messages['todo'] === 0)
				{
					...
					updateSettings(array('search_custom_index_resume' => json_encode(array_merge($context['index_settings'], array('resume_at' => $context['start'])), true)));
			}
	...
	if ($context['step'] === 3)
	{
		...
		updateSettings(array('search_index' => 'custom', 'search_custom_index_config' => json_encode($context['index_settings'], true)));
		$pmxcFunc['db_query']('', '
