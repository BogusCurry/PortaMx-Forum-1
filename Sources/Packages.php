					...
					if (!in_array(strtolower(strtr($real_path, array('\\' => '/'))), $themeFinds['other_themes']))
					{
						...
						if ($context['uninstalling'])
							$context['theme_actions'][$id]['actions'][] = array(
								...
								'description' => '',
								'value' => base64_encode(json_encode(array('type' => $action_data['type'], 'orig' => $action_data['filename'], 'future' => $real_path, 'id' => $id), true)),
								'not_mod' => true,
							);
						else
							$context['theme_actions'][$id]['actions'][] = array(
								...
								'action' => strtr($real_path, array('\\' => '/', $boarddir => '.')),
								'description' => '',
								'value' => base64_encode(json_encode(array('type' => $action_data['type'], 'orig' => $action_data['destination'], 'future' => $real_path, 'id' => $id), true)),
								'not_mod' => true,
							);
					}
				}
	...
	if (!empty($install_log))
	{
		...
		if (!$context['uninstalling'])
		{
			...
			if (!empty($db_package_log))
			{
				...
				$db_changes = json_encode($db_package_log, true);
			}
			else
				$db_changes = '';
			...
			// What failed steps?
			$failed_step_insert = json_encode($failed_steps, true);
			...
			// Credits tag?
			$credits_tag = (empty($credits_tag)) ? '' : json_encode($credits_tag, true);
			$pmxcFunc['db_insert']('',
...
function PackageBrowse()
{
	...
	$context['forum_version'] = str_replace('PortaMx-Forum', 'PMX', $forum_version);
...
function list_getPackages($start, $items_per_page, $sort, $params)
{
	...
	$the_version = strtr($forum_version, array('PortaMx-Forum ' => 'PMX '));
...
function PackagePermissions()
{
	...
	$context['back_look_data'] = base64_encode(json_encode(array_slice($context['look_for'], 0, 15), true));
	...
	if ($context['method'] == 'individual')
	{
		...
		if (isset($_POST['permStatus']))
		{
			...
			if (empty($context['to_process']))
				redirectexit('action=admin;area=packages;sa=perms' . (!empty($context['back_look_data']) ? ';back_look=' . base64_encode(json_encode($context['back_look_data'], true)) : '') . ';' . $context['session_var'] . '=' . $context['session_id']);
		}
