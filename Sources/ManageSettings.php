...
function ModifyBasicSettings($return_config = false)
{
	...
	$config_vars = array(
		...
		'',
			...
			array('text', 'default_personal_text', 'subtext' => $txt['default_personal_text_note'], 'size' => 30, 'disabled' => !$can_personal_text),
			array('check', 'topic_move_any'),
			array('int', 'defaultMaxListItems', 'step' => 1, 'min' => 1, 'max' => 999),
			array('text', 'disclaimer_link', null, 'size' => 40),
		'',
			// Jquery source
		...
		'',
			...
			(empty($modSettings['sef_enabled']) ? array('check', 'queryless_urls', 'subtext' => '<strong>' . $txt['queryless_urls_note'] . '</strong>') : array()),
...
function EditCustomProfiles()
{
	...
	if (isset($_POST['delete']) || isset($_POST['save']))
	{
		...
		updateSettings(array('displayFields' => json_encode($fields, true)));
		$_SESSION['adm-save'] = true;
