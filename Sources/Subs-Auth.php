...
function setLoginCookie($cookie_length, $id, $password = '')
{
	...
	if (isset($_COOKIE[$cookiename]) && preg_match('~^a:[34]:\{i:0;i:\d{1,7};i:1;s:(0|128):"([a-fA-F0-9]{128})?";i:2;[id]:\d{1,14};(i:3;i:\d;)?\}$~', $_COOKIE[$cookiename]) === 1)
	{
		...
		if (isset($array[3]) && $array[3] != $cookie_state)
		{
			...
			pmx_setcookie($cookiename, json_encode(array(0, '', 0), true), time() - 3600, $cookie_url[1], $cookie_url[0]);
		}
	}

	// Get the data and path to set it on.
	$data = json_encode(empty($id) ? array(0, '', 0) : array($id, $password, time() + $cookie_length, $cookie_state), true);
...
function setTFACookie($cookie_length, $id, $secret, $preserve = false)
{
	...
	// Get the data and path to set it on.
	$data = json_encode(empty($id) ? array(0, '', 0, $cookie_state, false) : array($id, $secret, time() + $cookie_length, $cookie_state, $preserve), true);
	$cookie_url = url_parts(!empty($modSettings['localCookies']), !empty($modSettings['globalCookies']));
