...
function ModBlockReportedPosts()
{
	...
	$cachekey = json_encode((int) $user_info['mod_cache']['bq']);
	$context['reported_posts'] = array();
	if ($user_info['mod_cache']['bq'] == '0=1')
		return 'reported_posts_block';

	if (($reported_posts = $pmxCacheFunc['get']('reported_posts' . $cachekey)) === null)
	{
		...
		// Cache it.
		$pmxCacheFunc['put']('reported_posts' . $cachekey, $reported_posts, 90);
	}
...
function ModBlockReportedMembers()
{
	...
	// Got the info already?
	$cachekey = json_encode((int) allowedTo('moderate_forum'));
...
function ViewWarningLog()
{
	...
	$context['search_params'] = empty($search_params['string']) ? '' : base64_encode(json_encode($search_params, true));
