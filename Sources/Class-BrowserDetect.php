	...
	*/
	function isChrome()
	{
		if (!isset($this->_browsers['is_Crome']))
			$this->_browsers['is_chrome'] = strpos($_SERVER['HTTP_USER_AGENT'], 'Chrome/') !== false && strpos($_SERVER['HTTP_USER_AGENT'], 'OPR/') === false;
	...
	*/
	function isOpera()
	{
		if (!isset($this->_browsers['is_opera']))
			$this->_browsers['is_opera'] = strpos($_SERVER['HTTP_USER_AGENT'], 'Opera') !== false || strpos($_SERVER['HTTP_USER_AGENT'], 'OPR/') !== false;
	...
	*/
	...
	private function setupOpera()
	{
		...
		elseif (preg_match('~Opera[ /]([0-9]+)(?!\\.[89])~', $_SERVER['HTTP_USER_AGENT'], $match))
			$this->_browsers['is_opera' . (int) $match[1]] = true;
        elseif(preg_match('~OPR\/[0-9]+\.[0-9]+\.[0-]+\.[0-9]+~', $_SERVER['HTTP_USER_AGENT'], $match))
            $this->_browsers['is_opera' . (int) $match[1]] = true;
