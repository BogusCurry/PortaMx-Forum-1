...
body{display:none !important;}');
	...
	addInlineJavascript('
	var footerHeight='. (!empty($modSettings['showCacheStatus']) && !empty($modSettings['timeLoadPageEnable']) ? '100' : (!empty($modSettings['showCacheStatus']) || !empty($modSettings['timeLoadPageEnable']) ? '80' : '60')) .';
	...
	document.onreadystatechange=function(){if(document.readyState==\'interactive\'){fSetFavicon();portamx_onload();}}');
		...
		{
			if(!in_array($name, array('PHPSESSID', 'eclauth', 'screen', 'currentVersion', $cookiename)))
			{
				$tmp[0] .= '\''. $name .'\',';
				$tmp[1] .= '\''. $value .'\',';
			}
