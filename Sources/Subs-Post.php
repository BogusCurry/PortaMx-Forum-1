...
function createPost(&$msgOptions, &$topicOptions, &$posterOptions)
{
	...
	{
		// ignore  mentions in a quote ..
		$mentions = array();
		preg_match_all('/\[quote[^\]]*](.*)\[\/quote\]/U', $msgOptions['body'], $quotes);
		if (isset($quotes[1]))
		{
			foreach($quotes[1] as $data)
			{
				$temp = Mentions::getMentionedMembers($data);
				if (is_array($temp) && count($temp) > 0)
					$msgOptions['body'] = str_replace($data, Mentions::getBody($data, $temp), $msgOptions['body']);
			}
		}
		...
		// remove the poster itself from mentions
...
function modifyPost(&$msgOptions, &$topicOptions, &$posterOptions)
{
	...
	if (isset($msgOptions['body']))
	{
		...
		if (!empty($modSettings['search_custom_index_config']) || !empty($modSettings['enable_mentions']))
		{
			$request = $pmxcFunc['db_query']('', '
				SELECT body
	...
	if (!empty($modSettings['enable_mentions']) && !empty($msgOptions['body']))
	{
		...
		if (!empty($msgOptions['old_body']))
		{
			...
			{
				foreach($match[1] as $i => $memID)
					$oldmentions[$memID] = array('id' => $memID, 'real_name' => $match[2][$i]);
			}
