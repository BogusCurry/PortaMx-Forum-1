...
function upgradeExit($fallThrough = false)
{
	...
	if (!$fallThrough)
	{
		...
		if (isset($upcontext['sub_template']))
		{
			...
			$upcontext['form_url'] = $upgradeurl . '?step=' . $upcontext['current_step'] .  (isset($_GET['substep']) ? '&amp;substep=' . $_GET['substep'] : '') . '&amp;data=' . base64_encode(safe_serialize($upcontext['upgrade_status']));
...
function initialize_inputs()
{
	...
	if (isset($_GET['delete']))
	{
		...
		// 2.0 Sources files not in PortaMx Forum
		...
		// 2.0 Template files not in PortaMx Forum
		...
		// 2.0 Language files not in PortaMx Forum
			...
			if ($table == 'settings')
			{
				...
				foreach ($serialized_settings as $var)
				{
					if (isset($modSettings[$var]))
					{
						...
						elseif ($temp !== false)
							$new_settings[$var] = json_encode($temp, true);
					}
				}
			...
			elseif ($table == 'themes')
			{
				...
				if ($pmxcFunc['db_num_rows']($query) > 0)
				{
					while ($row = $pmxcFunc['db_fetch_assoc']($query))
					{
						...
						if ($temp !== false)
						{
							$row['value'] = json_encode($temp, true);
...
{
	global $upcontext, $disable_security, $settings, $modSettings, $txt;

	$forum_version = PMX_VERSION;
	$temp = file_get_contents(dirname(__FILE__) .'/Settings.php');
	preg_match('/version\s+([a-zA-Z0-9\.\s]+)/', $temp, $match);
	if(isset($match[1]) && trim($match[1]) == $forum_version)
	{
		echo '
			<h3>Your Forum is allready up to date.<br>The current version is <b>'. PMX_FULL_VERSION .'</b></h3>';
		exit;
	}

	echo '
			<h3>', sprintf($txt['upgrade_ready_proceed'], PMX_FULL_VERSION), '</h3>
    ...
    if(!file_exists($settings['default_theme_dir'] . '/scripts/script.js'))
	    echo '
		<div style="margin: 2ex; padding: 2ex; border: 2px dashed #804840; color: black; background-color: #fe5a44;" id="js_script_missing_error">
