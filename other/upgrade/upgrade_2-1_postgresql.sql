...
CREATE TABLE IF NOT EXISTS {$db_prefix}portal_settings (
...
CREATE UNIQUE INDEX IF NOT EXISTS {$db_prefix}portal_settings_uidx ON {$db_prefix}portal_settings (varname);
---#

---# Sequence for table `portal_blocks`
CREATE SEQUENCE IF NOT EXISTS {$db_prefix}portal_blocks_seq;
---#

---# Table structure for table `portal_blocks`
CREATE TABLE IF NOT EXISTS {$db_prefix}portal_blocks (
	id int default nextval('{$db_prefix}portal_blocks_seq'),
...
---# Indexes for table `portal_blocks`
CREATE INDEX IF NOT EXISTS {$db_prefix}portal_blocks_sidepos ON {$db_prefix}portal_blocks (side, pos);
CREATE INDEX IF NOT EXISTS {$db_prefix}portal_blocks_blocktype ON {$db_prefix}portal_blocks (blocktype);
---#

---# Table structure for table `portal_categories`
CREATE TABLE IF NOT EXISTS {$db_prefix}portal_categories (
...
---# Indexes for table `portal_categories`
CREATE INDEX IF NOT EXISTS {$db_prefix}portal_categories_name ON {$db_prefix}portal_categories (name);
CREATE INDEX IF NOT EXISTS {$db_prefix}portal_categories_catorder ON {$db_prefix}portal_categories (catorder);
---#

---# Table structure for table `portal_articles`
CREATE TABLE IF NOT EXISTS {$db_prefix}portal_articles (
	id INT(10) UNSIGNED NOT NULL,
	name VARCHAR(80) NOT NULL DEFAULT '',
...
---# Indexes for table `portal_categories`
CREATE INDEX IF NOT EXISTS {$db_prefix}portal_articles_name ON {$db_prefix}portal_articles (name);
CREATE INDEX IF NOT EXISTS {$db_prefix}portal_articles_catid ON {$db_prefix}portal_articles (catid);
CREATE INDEX IF NOT EXISTS {$db_prefix}portal_articles_actapp ON {$db_prefix}portal_articles (active, approved);
---#
...
<ul class="list_plus">
	<li>PortaMx-Forum is a free and powerful Forum with a integrated&nbsp;Portal</li>
	<li>PortaMx-Forum is fast, modular and simple to use</li>
	<li>PortaMX-Forum have full support for mobile devices</li>
	<li>PortaMx-Forum is full compatible with the <b><a class="bbc_link" href="http://ec.europa.eu/ipg/basics/legal/cookies/index_en.htm" target="_blank">EU legislation on cookie law</a></b></li>
</ul>
...
This structure reduces unnecessary loads and save memory, because the block is only loaded, if it is visible and the code not loaded.<br />
All settings are stored in the database, so we can add new settings without any change on the database tables.<br />
...
<ul class="list_go">
	...
	<li>ECL&nbsp;settings (<span style="margin: 0px; padding: 0px; border: 0px currentColor; border-image: none; font-family: inherit; font-style: inherit;">EU legislation on cookies)</span></li>
...
<ul class="list_red">
	<li>System blocks like Recent posts, <span style="font-family: Tahoma; font-size: small;">Recent topics,&nbsp;</span>User, Statistic, RSS Reader and mutch&nbsp;more</li>
...
VALUES
	...
	('sef_actions', 'about:mozilla,about:unknown,activate,announce,attachapprove,buddy,calendar,clock,collapse,community,coppa,credits,deletemsg,display,dlattach,editpoll,editpoll2,emailuser,findmember,groups,help,helpadmin,im,jseditor,jsmodify,jsoption,keepalive,language,lock,lockvoting,login,login2,logout,markasread,mergetopics,mlist,moderate,modifycat,modifykarma,movetopic,movetopic2,notify,notifyboard,openidreturn,pm,post,post2,printpage,profile,promote,quotefast,quickmod,quickmod2,recent,register,register2,reminder,removepoll,removetopic2,reporttm,requestmembers,restoretopic,search,search2,sendtopic,signup,signup2,smstats,suggest,spellcheck,splittopics,stats,sticky,themes,trackip,unread,unreadreplies,verificationcode,viewprofile,vote,viewquery,viewsmfile,who,.xml,xmlhttp,theme,notifytopic,likes,signup,loadeditorlocale,xml'),
	('sef_autosave', '0'),
	('sef_enabled', '0'),
	('sef_ignoreactions', ''),
	('sef_lowercase', '1'),
...
WHERE filename IN ('angel.gif', 'police.gif');
...
/******************************************************************************/
--- Update Messageicons table
/******************************************************************************/
---# Update Message Icons
DELETE FROM {$db_prefix}message_icons
	WHERE filename != '';

INSERT IGNORE INTO {$db_prefix}message_icons
