...
#
CREATE UNIQUE INDEX {$db_prefix}portal_settings_uidx ON {$db_prefix}portal_settings (varname);

#
# Sequence for table `portal_blocks`
#
CREATE SEQUENCE {$db_prefix}portal_blocks_seq;
...
#
CREATE INDEX {$db_prefix}portal_blocks_sidepos ON {$db_prefix}portal_blocks (side, pos);
CREATE INDEX {$db_prefix}portal_blocks_blocktype ON {$db_prefix}portal_blocks (blocktype);
...
#
CREATE INDEX {$db_prefix}portal_categories_name ON {$db_prefix}portal_categories (name);
CREATE INDEX {$db_prefix}portal_categories_catorder ON {$db_prefix}portal_categories (catorder);
...
# Indexes for table `portal_categories`
#
CREATE INDEX {$db_prefix}portal_articles_name ON {$db_prefix}portal_articles (name);
CREATE INDEX {$db_prefix}portal_articles_catid ON {$db_prefix}portal_articles (catid);
CREATE INDEX {$db_prefix}portal_articles_actapp ON {$db_prefix}portal_articles (active, approved);
...
{$default_portamx_smileyset_name}'),
	...
	('sef_actions', 'about:mozilla,about:unknown,activate,announce,attachapprove,buddy,calendar,clock,collapse,community,coppa,credits,deletemsg,display,dlattach,editpoll,editpoll2,emailuser,findmember,groups,help,helpadmin,im,jseditor,jsmodify,jsoption,keepalive,language,lock,lockvoting,login,login2,logout,markasread,mergetopics,mlist,moderate,modifycat,modifykarma,movetopic,movetopic2,notify,notifyboard,openidreturn,pm,post,post2,printpage,profile,promote,quotefast,quickmod,quickmod2,recent,register,register2,reminder,removepoll,removetopic2,reporttm,requestmembers,restoretopic,search,search2,sendtopic,signup,signup2,smstats,suggest,spellcheck,splittopics,stats,sticky,themes,trackip,unread,unreadreplies,verificationcode,viewprofile,vote,viewquery,viewpmxfile,who,.xml,xmlhttp,theme,notifytopic,likes,signup,loadeditorlocale,xml'),
	('sef_autosave', '0'),
	('sef_enabled', '0'),
	('sef_ignoreactions', ''),
	('sef_lowercase', '1'),
...
VALUES ('Google', 'googlebot', ''),
	...
	('Yandex', 'yandex', ''),
	('PortaMx Spidertest', 'PortaMx Spider', '');
#---------------------------------------------------------
