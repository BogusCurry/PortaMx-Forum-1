...
function InitDropdownHandler(elm)
{
	...
	pWind.dropdown.elm.className = 'dropdown';
	pWind.dropdown.elm.onmousedown = function(){setHandleDropdown();};
	pWind.dropdown.optHeight = parseInt($('.dropdown option').css('height'));
	pWind.dropdown.ignore_click = !isFireFox;
}

// set a special select dropdown handling
...
function setHandleDropdown()
{
	...
	pWind.dropdown.closedHeight = rect.height;
	pWind.dropdown.elm.style.height = pWind.dropdown.closedHeight +'px';
	pWind.dropdown.elm.size = pWind.dropdown.elm.options.length > 10 ? 10 : (pWind.dropdown.elm.options.length > 2 ? pWind.dropdown.elm.options.length -1 : 2);
	aniheight = (pWind.dropdown.elm.size * pWind.dropdown.optHeight) + (pWind.dropdown.elm.options.length > 2 ? pWind.dropdown.closedHeight + 2 : 6) +'px';
...
/*
	get the popwindow position
	ofsXY can by: nn or -nn
	means offset from left and top
*/
function pmx_PopUpPos(posID, ofsXY, elm, fitRight)
{
	eval('var ofs={'+ ofsXY +'}');
	var yofs = 24 + parseInt(ofs.y);
	...
	if(fitRight)
	{
		var xofs = parseInt(ofs.x) + 6;
		var width = document.getElementById('func-row').offsetWidth;
		var left = pWind.style.width.match(/[0-9]+/);
		pWind.style.left = width - (parseInt(left[0]) + xofs) +'px';
		pWind.style.top = Math.round(yofs + (elmRect.top - formRect.bottom) - 1) +'px';
	}
	else
	{
		var xofs = parseInt(ofs.x) - 1;
		pWind.style.left = xofs +'px';
		pWind.style.top = Math.round(yofs + (elmRect.top - formRect.bottom)) +'px';
	}

	// show the popup
	$(pWind).slideDown('fast');
}
...
function SetpmxBlockType(side, sidedesc, poselm)
{
	...
	if(pWind)
	{
		...
		// show the popup
		pmx_PopUpPos(side, 'x:-5,y:35', poselm, true);
...
function pmxRowMove(id, side, poselm)
{
	...
	if(allid.length > 1)
	{
		...
		if(pWind)
		{
			...
			// show the popup
			pmx_PopUpPos(id, 'x:0,y:0', poselm, false);
...
function pmxSetTitle(id, Bside, poselm)
{
	...
	if(pWind)
	{
		...
		// show the popup
		if(Bside == 'cat')
			pmx_PopUpPos(id, 'x:-1,y:0', poselm, false);
		else if(Bside == '')
			pmx_PopUpPos(id, 'x:-2,y:0', poselm, false);
		else
			pmx_PopUpPos(id, 'x:-2,y:0', poselm, false);
	}
}
...
function pmxSetAcs(id, pSide, poselm)
{
	...
	if(pWind)
	{
		...
		// show the popup
		if(side != 'cat')
			pmx_PopUpPos(id, 'x:0,y:0', poselm, true);
		else
			pmx_PopUpPos(id, 'x:-4,y:3', poselm, true);
...
function pmxSetFilter(poselm)
{
	...
	{
		// show the popup
		pmx_PopUpPos(0, 'x:-2,y:-11', poselm, false);
	}
}
...
function SetpmxArticleType(poselm)
{
	...
	{
		// show the popup
		pmx_PopUpPos('', 'x:-5,y:29', poselm, true);
...
function pmxRemovePopup()
{
	...
	{
		var side = document.getElementById('pWind.side').value.replace(/\./i, '');

		$(pWind).slideUp('fast', function(){window.setTimeout(1000, restorePopup())});
