...
function SendShout(sbID, state)
{
	if(state == 1)
	{
		...
		else
		{
			...
			{
				pmxCookie("set", "pmx_shout"+ sbID, "none");
...
{
	if(state == "check")
		state = pmxCookie("get", "pmx_shout"+ sbID, '', false);
	...
	if(state == "none")
	{
		if(document.getElementById("shout_toggle"+ sbID))
		{
			...
			if(document.getElementById("shoutcontdiv"+ sbID).style.display != 'block')
			{
				document.getElementById("shout_key"+ sbID).title = pmx_shoutbox_button_title;
				document.getElementById("shout_key"+ sbID).value = pmx_shoutbox_button_open;
			}
	...
	pmxCookie("set", "pmx_shout"+ sbID, state == "none" ? "block" : "none", '', false);
}
/* eof */