...
if (typeof(pmx_restore_top) == 'undefined')
	pmx_restore_top = false;
...
function reqOverlayDiv(desktopURL, sHeader, sIcon)
{
	...
	// is a spidertest performed?
	if(pmxCookie('get', 'spidertest') != '')
		return false;
					...
					&& (this.opt.oCookieOptions.sCookieName.match(/left/) || this.opt.oCookieOptions.sCookieName.match(/right/)))
			{
				if (typeof portamx_EqualHeight == 'function' && oContainer.id != 'upshrinkVisual')
				{
					if (bCollapse)
						...
						$(oContainer).show(400, function(){portamx_EqualHeight(true)});
				}
				else
				{
					if (bCollapse)
...
IconList.prototype.initIcons = function ()
{
	for (var i = document.images.length - 1, iPrefixLength = this.opt.sIconIdPrefix.length; i >= 0; i--)
		if (document.images[i].id.substr(0, iPrefixLength) == this.opt.sIconIdPrefix)
			setOuterHTML(document.images[i], '<div title="' + this.opt.sLabelIconList + '" onclick="' + this.opt.sBackReference + '.openPopup(this, ' + document.images[i].id.substr(iPrefixLength) + ')" onmouseover="' + this.opt.sBackReference + '.onBoxHover(this, true)" onmouseout="' + this.opt.sBackReference + '.onBoxHover(this, false)" style="background: ' + this.opt.sBoxBackground + '; cursor: pointer; padding: 3px 3px 0 3px; text-align: center;"><img src="' + document.images[i].src + '" alt="' + document.images[i].alt + '" id="' + document.images[i].id + '" style="margin: 0px; padding: 3px 0px 0px 0px;"></div>');
}
...
IconList.prototype.onBoxHover = function (oDiv, bMouseOver)
{
	...
	oDiv.style.padding = bMouseOver ? '2px 2px 0px 2px' : '3px 3px 1px 3px';
}
...
IconList.prototype.openPopup = function (oDiv, iMessageId)
{
	...
	if (!this.bListLoaded && this.oContainerDiv == null)
	{
		...
		this.oContainerDiv.style.padding = '2px 1px 4px 1px';
	...
	this.oContainerDiv.style.top = $(oDiv).offset().top + 24 +'px';
	this.oContainerDiv.style.left = $(oDiv).offset().left +'px';
...
IconList.prototype.onItemHover = function (oDiv, bMouseOver)
{
	...
	oDiv.style.padding = '1px 3px 5px';
...
IconList.prototype.onItemMouseDown = function (oDiv, sNewIcon)
{
	...
	{
		ajax_indicator(true);
		$.ajax({type: 'GET', async:false, url:pmx_scripturl + '?action=jsmodify;topic=' + this.opt.iTopicId + ';msg=' + this.iCurMessageId + ';' + pmx_session_var + '=' + pmx_session_id + ';icon=' + sNewIcon + ';xml', data:{}, success:function(data){sResult = data;}});
		ajax_indicator(false);

		var oMessage = sResult.getElementsByTagName('pmx')[0].getElementsByTagName('message')[0];
...
{
	return sUrl.indexOf('?') == -1 ? sUrl + '?' : sUrl;
}
...
function pmxCookie(sMode, sName, sValue, sType, bAsync)
{
	...
	if(sName.indexOf('pmx_YOfs') == -1 && sName !== 'screen' && sName !== 'spidertest')
	{
		...
		{
			if(allCookies.Name.indexOf(sName) >= 0)
			{
				if(allCookies.Value[allCookies.Name.indexOf(sName)] !== sValue.toString())
				{
					allCookies.Value[allCookies.Name.indexOf(sName)] = sValue.toString();
					bAsync = true;
				}
				else
					return;
			...
			{
				allCookies.Name[allCookies.Name.length] = sName;
				allCookies.Value[allCookies.Value.length] = (sValue == undefined ? '' : sValue.toString());
				bAsync = true;
			}
		}
	}
