...
function template_main()
{
	...
	if ($context['can_moderate_forum'] || $context['user']['is_mod'])
		...
		if($context['current_board'] != $modSettings['recycle_board'] && !empty($context['pmx']['can_promote']))
			echo '
					function toggle_promote(msgID)
					{
						ajax_indicator(true);
						newstate = pmxXMLpost("'. $scripturl .'?action=promote;message="+msgID);
						newstate = newstate.split(",");
						document.getElementById("pmx_promo_img_"+msgID).className = "pmx_"+ newstate[0] +"_promote";
						document.getElementById("pmx_promo_txt_"+msgID).innerText = newstate[1];
...
function template_single_post($message)
{
	...
	if (!empty($message['attachment']))
	{
		...
		foreach ($message['attachment'] as $attachment)
		{
			...
			echo '
											<br>', $attachment['size'], ($attachment['is_image'] ? ', ' . $attachment['real_width'] . 'x' . $attachment['real_height'] : '<br>' . sprintf($txt['attach_downloaded'], $attachment['downloads'])), '
										</div>';
	...
	if($context['can_report_moderator'] || (!empty($modSettings['enable_likes']) && !empty($context['can_see_likes']) && !empty($message['likes']['count'])) || $message['can_approve'] || $message['can_unapprove'] || $context['can_reply'] || $message['can_modify'] || $message['can_remove'] || $context['can_split'] || $context['can_restore_msg'] || $context['can_quote'])
	{
		...
		if ($message['can_approve'] || $message['can_unapprove'] || $context['can_reply'] || $message['can_modify'] || $message['can_remove'] || $context['can_split'] || $context['can_restore_msg'] || $context['can_quote'])
		{
			...
			if(!empty($promo))
				echo '
									<li><a id="pmx_promo_img_'. $message['id'] .'" class="pmx_'. $promo .'_promote" href="javascript:void(0)" onclick="toggle_promote('. $message['id'] .')"><span id="pmx_promo_txt_'. $message['id'] .'" class="pmx_promote_txt">'. $txt['pmx_'. $promo .'_promote'] .'</span></a></li>';
